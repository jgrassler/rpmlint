...
class PythonCheck(AbstractFilesCheck):
    ...
    def check_egginfo(self, pkg, filename):
        ...
        requires_path = os.path.join(pkg.dirName(), filename.lstrip('/'), 'requires.txt')
        relative_requires_path = os.path.join(filename, 'requires.txt')
        if os.path.isfile(requires_path):
            self.compare_requires(pkg, requires_path, relative_requires_path)

    def compare_requires(self, pkg, requires_path, relative_requires_path):
        """
        ...
        for egg_require in from_egg:
            ...
            if not found:
                self.output.add_info('W', pkg, WARNS['req-missing'], relative_requires_path, egg_require)
